...
FROM (
    ...
    SELECT
        ...
        FALSE AS is_group,
        u.username AS name,
        COALESCE(m.id, '') AS last_message_id,
    ...
    JOIN conversation_members cm2 ON cm2.conversation_id = c.id AND cm1.user_id != cm2.user_id
    ...
    WHERE
        ...
        AND EXISTS (
            ...
            FROM friends f
            WHERE
                (f.user_id = cm1.user_id AND f.friend_id = cm2.user_id)
                OR (f.user_id = cm2.user_id AND f.friend_id = cm1.user_id)
        )
